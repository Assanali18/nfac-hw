...
class AuthService {
  ...
  private generateJwt(user: IUser): string {
    return jwt.sign({ id: user._id, username: user.username }, this.jwtSecret, { expiresIn: '1h' });
  }

  private generateRefreshToken(user: IUser): string {
    return jwt.sign({ id: user._id, username: user.username }, this.jwtRefreshSecret, { expiresIn: '7d' });
  }
